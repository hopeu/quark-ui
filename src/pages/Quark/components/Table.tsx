...
const Table: React.FC<Table> = (props: any) => {
  ...
  const columnRender = (column: any, text: any) => {
    ...
    if (column.qrcode) {
      const img: any = <img src={text} width={column.qrcode.width} height={column.qrcode.height}/>;
      columnComponent =
  ...
  const getColumnsStateValues = () => {
    const values = {};
    props.table.columns.forEach((item) => {
      if (item.hide) {
        values[`${item.dataIndex}`] = {show: false};
      }
    })
    return values;
  }

  const [columnsStateMap, setColumnsStateMap] = useState<Record<string, ColumnsState>>(getColumnsStateValues());
    ...
    <>
      ...
      <ProTable
        ...
        tableAlertOptionRender={({selectedRowKeys, onCleanSelected}) => {
          return (
            <BatchAction actions={props.table.batchActions} selectedRowKeys={selectedRowKeys}
                         onCleanSelected={onCleanSelected} current={actionRef.current}/>
          );
        }}
        ...
        pagination={{
          defaultPageSize: props.table.pagination.pageSize,
          showSizeChanger: true,
          pageSizeOptions: ['10', '20', '50', '100', '200', '500', '1000'],
          // current: props.table.pagination.current,
          defaultCurrent: props.table.pagination.current
        }}
        dateFormatter={props.table.dateFormatter}
        columnEmptyText={props.table.columnEmptyText}
        toolbar={{
          multipleLine: false,
          actions: props.table.toolbar.actions.length > 0 ? [<ToolBarAction key={props.table.toolbar.key}
                                                                            actions={props.table.toolbar.actions}
                                                                            current={actionRef.current}/>] : undefined,
        }}
